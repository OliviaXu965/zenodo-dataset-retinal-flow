function [speed_map, speed_hists, dir_map, dir_hist] = get4PanelMeanDataSlipFixations_set_gain()
...
% if exist(vis_mag_df_path)~=2
    vis_mag = computeVisMagDf(load([dataSource '/mesh_orig_masks_saccades_hist_all_set_gain.mat']).histsAll,'mesh');
...
% if exist(vis_dir_df_path)~=2
    vis_dir = computeVisDirDf(load([dataSource '/mesh_orig_masks_saccades_hist_all_set_gain.mat']).histsAll,'mesh');
...
%%
lstruct=load([dataSource '/mesh_orig_masks_saccades_hist_all_set_gain.mat']);
